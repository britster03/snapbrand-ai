...
class ApiClient {
  ...
  async login(credentials: UserLogin): Promise<Token> {
    const response = await this.request<Token>('/v1/auth/login/json', {
      method: 'POST',
  ...
  async getApiKey(): Promise<{ api_key: string }> {
    return this.request('/v1/auth/api-key');
  }
  ...
  async listUserImages(params: {
    ...
    return this.request(`/v1/images?${queryParams}`);
  }

  async getImage(imageId: string): Promise<GeneratedImage> {
    return this.request(`/v1/images/${imageId}`);
  }
...
// Export the class for manual instantiation in frontend code
export { ApiClient };
